...
from .models import Clubs, User, SportsFacility
...
def home(request):
    ...
    if ctprvn_nm:
        clubs = clubs.filter(ctprvn_nm__icontains=ctprvn_nm)
        facilities = facilities.filter(city__icontains=ctprvn_nm) # 시도명 필터
    if signgu_nm:
        clubs = clubs.filter(signgu_nm__icontains=signgu_nm)
        if signgu_nm == "전체":
            facilities = facilities.filter(city__icontains=ctprvn_nm)
        else:
            facilities = facilities.filter(city__icontains=ctprvn_nm, district__icontains=signgu_nm)# 시군구명 필터
    if item_nm:
